...
void init_app() {
    ...
    if (fp == NULL) {
        puts("Our secret article is leaked. Please ask a new one in @ugractf.");
        exit(1);
    }
    ...
    if (fp == NULL) {
        puts("Chief editor is fired. Please hire a new one in @ugractf.");
        exit(1);
    }
